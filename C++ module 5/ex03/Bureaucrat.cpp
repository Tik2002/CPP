...
void Bureaucrat::decGrade() {++this->grade;validate(grade);}

void Bureaucrat::incGrade() {--this->grade;validate(grade);}
...
void Bureaucrat::executeForm(AForm& form)
{
	if (this->getGrade() > form.getSignGrade())
	{
		cout << this->getName() << " couldn't execute " << form.getName() << " because hasn't permission." << endl;
		return ;
	}
	int _signed = form.getSign();
	switch (_signed)
	{
		case 0:
			cout << this->getName() << " couldn't execute " << form.getName() << " because form doesn't signed." << endl;
			break;
		default:
			form.execute(*this);
	}
